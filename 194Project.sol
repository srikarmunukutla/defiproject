...
contract Election {
    ...
    mapping(address => Voter) public voters; // Mapping of people who have already voted

    mapping(bytes32 => Candidate) public candidates; //Mapping of the candidates running
     ...
     * @param candidate the name of the candidate that the voter would like to vote for.
     */
    function vote(uint voterId, bytes32 candidateName) public {
        ...
        require(bytes(voters[hashedVoterID]).length <= 0, "Already voted.");

        //Create a voter object for the voter with the given voterId. Put it in storage.
        Voter storage currentVoter = Voter({
        ...
        if (bytes(candidates[candidateName]).length <=0) {
            candidates[candidateName] = Candidate({
                name: candidateName,
                voteCount: 0
            });
        }
        //Increase the number of votes for the given candidate by 1.
        candidates[candidateName] = candidates[candidateName].voteCount + 1;
            ...
            returns (bytes32 winningCandidate_)
    {
        ...
        for (uint i = 0; i < candidates.length; i++) {
            if (candidates[i].voteCount > maxVoteCount) {
                ...
                winningCandidate_ = candidates[i].name;
            }
        }
    }
}